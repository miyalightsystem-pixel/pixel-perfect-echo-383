...
import { Plus, Trophy, FolderOpen, Pencil, Trash2 } from "lucide-react";
...
import {
  ...
  markPersonalDone,
  setTugasStatus,
} from "@/lib/empire.functions";
import { useActiveMember, canManageTugas } from "@/lib/active-member";
...
function BalaiTugas() {
  ...
  const canManage = canManageTugas(member?.role);

  const createFn = useServerFn(createTugas);
  const updateFn = useServerFn(updateTugas);
  const deleteFn = useServerFn(deleteTugas);
  ...
  const invalidateTugas = () => qc.invalidateQueries({ queryKey: ["tugas"] });

  const createMut = useMutation({
    mutationFn: (input: { judul: string; matkul: string; deadline: string; actor_id?: string }) =>
      createFn({ data: input }),
    onSuccess: () => {
      invalidateTugas();
  ...
  const updateMut = useMutation({
    mutationFn: (input: { id: string; judul: string; matkul: string; deadline: string; actor_id: string }) =>
      updateFn({ data: input }),
    onSuccess: () => {
      invalidateTugas();
      toast.success("Titah tugas diperbarui.");
      setEditing(null);
    },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  const deleteMut = useMutation({
    mutationFn: (input: { id: string; actor_id: string }) => deleteFn({ data: input }),
    onSuccess: () => {
      invalidateTugas();
      toast.success("Tugas dihapus dari titah kerajaan.");
      setDeletingId(null);
    },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  const leaderboard = useMemo(() => {
    ...
    for (const c of completions as Array<{
      ...
      if (!row) continue;
      row.total += 1;
      if (c.tugas && new Date(c.completed_at) <= new Date(c.tugas.deadline)) row.tepat += 1;
    }
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-2">
          ...
          {canManage && (
            <TugasDialog
              ...
              onSubmit={(d) => createMut.mutate({ ...d, actor_id: member?.id })}
            />
          )}
        </div>
      </header>
      ...
      <section className="space-y-3">
        {tugas && tugas.length > 0 ? (
          tugas.map((t) => {
            ...
            return (
              <article key={t.id} className="rounded-xl border bg-card p-4 flex items-start gap-3">
                <Checkbox
                  checked={done}
                  disabled={!member}
                  onCheckedChange={(v) =>
                    member && markMut.mutate({ tugas_id: t.id, anggota_id: member.id, done: !!v })
                  }
                  className="mt-1"
                />
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-2 flex gap-1.5 flex-wrap">
                    {(["belum", "dikerjakan", "selesai"] as const).map((s) => (
                      <button
                        key={s}
                        onClick={() => statusMut.mutate({ id: t.id, status: s, actor_id: member?.id })}
        ...
        {leaderboard.length > 0 ? (
          <ol className="mt-3 space-y-1.5">
            {leaderboard.map((row, i) => (
              <li key={row.nama} className="flex items-center justify-between gap-2 rounded-lg bg-cream/60 px-3 py-2">
      ...
      {editing && (
        <TugasDialog
          ...
          initial={editing}
          onSubmit={(d) =>
            member && updateMut.mutate({ id: editing.id, ...d, actor_id: member.id })
          }
        />
      )}
      ...
      <AlertDialog open={!!deletingId} onOpenChange={(o) => !o && setDeletingId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Hapus titah tugas ini?</AlertDialogTitle>
            <AlertDialogDescription>
              Tindakan tidak bisa dibatalkan. Semua riwayat penyelesaian tugas ini akan hilang.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => deletingId && member && deleteMut.mutate({ id: deletingId, actor_id: member.id })}
            >
              Hapus
            </AlertDialogAction>
          </AlertDialogFooter>
...
function TugasDialog({
  ...
  initial?: { judul: string; matkul: string; deadline: string };
  open?: boolean;
  ...
  useEffect(() => {
    if (isOpen && initial) {
      ...
      // datetime-local format: YYYY-MM-DDTHH:mm (lokal)
  ...
  const handleSubmit = () => {
    ...
    if (new Date(iso) < new Date()) {
      toast.warning("Deadline sudah lewat — yakin tetap simpan?", {
        action: {
          label: "Tetap simpan",
          onClick: () => {
            onSubmit({ judul, matkul, deadline: iso });
            setOpen(false);
          },
        },
      });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="font-display text-empire">
            {mode === "create" ? "Sebar Titah Tugas Baru" : "Ubah Titah Tugas"}
          </DialogTitle>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit}>
            {mode === "create" ? "Sebar Titah" : "Simpan Perubahan"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
